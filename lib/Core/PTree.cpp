...
llvm::cl::opt<bool>
    WritePTree("write-ptree", llvm::cl::init(false),
               llvm::cl::desc("Write process tree into ptree.db"),
...
PTreeNode::PTreeNode(PTreeNode *parent, ExecutionState *state,
                     NodeType nt) noexcept
    : parent{parent}, left{nullptr}, right{nullptr}, state{state}, nodeType{
                                                                       nt} {
  state->ptreeNode = this;
}

PTreeNode::PTreeNode(PTreeNode *parent, ExecutionState *state) noexcept
    : PTreeNode(parent, state, NT_Basic) {}

// AnnotatedPTreeNode

AnnotatedPTreeNode::AnnotatedPTreeNode(PTreeNode *parent,
                                       ExecutionState *state) noexcept
    : PTreeNode(parent, state, NT_Annotated) {
  id = nextID++;
}

// NoopPTree

NoopPTree::NoopPTree() noexcept : PTree(PT_Noop) {}
...
InMemoryPTree::InMemoryPTree(ExecutionState &initialState,
                             PTreeType pt) noexcept
    : PTree{pt} {}
...
PersistentPTree::PersistentPTree(ExecutionState &initialState,
    ...
    : InMemoryPTree(initialState, PT_Persistent),
...
void PersistentPTree::setTerminationType(ExecutionState &state,
  ...
  auto *annotatedNode = llvm::dyn_cast<AnnotatedPTreeNode>(state.ptreeNode);
  assert(annotatedNode);
  annotatedNode->kind = type;
}

void PersistentPTree::updateBranchingNode(PTreeNode &node, BranchType reason) {
  auto *annotatedNode = llvm::dyn_cast<AnnotatedPTreeNode>(&node);
  assert(annotatedNode);
...
std::unique_ptr<PTree> klee::getPTree(ExecutionState &state, bool inMemory,
                                      InterpreterHandler &ih) {
  if (WritePTree)
    return std::make_unique<PersistentPTree>(state, ih);

  if (inMemory)
    return std::make_unique<InMemoryPTree>(state);
